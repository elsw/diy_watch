...
void RTC_I2C::write_register(uint8_t reg, uint8_t val) {
  ...
  i2c_write_blocking(i2c_dev, address, buffer, 2, false);
}
...
uint8_t RTC_I2C::read_register(uint8_t reg) {
  uint8_t buffer[1] = {0};
  //i2c_dev->write(&reg, 1);
  i2c_write_blocking(i2c_dev, address, &reg, 1, false);
  //i2c_dev->read(buffer, 1);
  i2c_read_blocking(i2c_dev, address, buffer, 1, false);
  return buffer[0];
}
